...
public class FitnessFunction {
    ...
    public double calculateResourceUtilization(int[] allocation) {
        double totalExecutionTime = 0;
        for (int i = 0; i < cloudletList.size(); i++) {
            Cloudlet cloudlet = cloudletList.get(i);
            Vm vm = vmList.get(allocation[i]);
            totalExecutionTime += cloudlet.getCloudletLength() / vm.getMips();
        }
        return totalExecutionTime / (vmList.size() * calculateMakespan(allocation));
    }
    ...
    public double calculateFitness(int[] allocation) {
        ...
        double resourceUtilization = calculateResourceUtilization(allocation);
        double degreeOfImbalance = calculateDegreeOfImbalance(allocation);
        
        // Fungsi fitness: Meminimalkan makespan dan degree of imbalance, maksimalkan resource utilization
        return makespan + degreeOfImbalance - resourceUtilization;
    }
}